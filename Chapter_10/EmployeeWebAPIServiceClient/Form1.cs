...
namespace EmployeeWebAPIServiceClient
{
    public partial class Form1 : Form
    {
        ...
        {
            HttpResponseMessage response = client.GetAsync("/api/EmployeeManager/" + comboBox1.SelectedItem).Result;
            string xmlData = response.Content.ReadAsStringAsync().Result;
        ...
        {
            string xmlEmp = $"<Employee xmlns='http://schemas.datacontract.org/2004/07/EmployeeWebAPIService.Models' xmlns:i='http://www.w3.org/2001/XMLSchema-instance'><FirstName>{textBox1.Text}</FirstName><HomePhone>{textBox3.Text}</HomePhone><LastName>{textBox2.Text}</LastName><Notes>{textBox4.Text}</Notes></Employee>";


            //string xmlEmp = $"<Employee><FirstName>{textBox1.Text}</FirstName><HomePhone>{textBox3.Text}</HomePhone><LastName>{textBox2.Text}</LastName><Notes>{textBox4.Text}</Notes></Employee>";

            HttpContent content = new StringContent(xmlEmp, Encoding.UTF8, "application/xml");

            HttpResponseMessage response = client.PostAsync("/api/EmployeeManager", content).Result;
        ...
        {
            HttpResponseMessage response = client.DeleteAsync("/api/EmployeeManager/" + comboBox1.SelectedItem).Result;
